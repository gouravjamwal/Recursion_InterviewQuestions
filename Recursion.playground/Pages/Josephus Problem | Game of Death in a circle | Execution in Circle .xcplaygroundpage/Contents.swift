...
func josephusProblem() {
    ...
    print(array)
    let lastOneStanding = solve(array: &array, k: k, index: 0)
    print("lastOneStanding --> \(lastOneStanding)")
}

func solve(array: inout [Int], k: Int, index: Int) -> Int {
    if array.count == 1 {
        return array[0]
    }
    let index = (index + k - 1) % array.count
    
    print("Killed soldier Number -> \(array.remove(at: index))")
    return solve(array: &array, k: k, index: index)
}
josephusProblem()
